...
export class BitUtil {
     ...
     */
    static packBits(packMap, sourceData, warnNotNormal = true) {
     ...
     *  by the packMap.
     **/
    static unpackBits(packMap, bits, warnNotNormal = true) {
        return BitUtil.normalizePackMap(packMap, warnNotNormal)
            .map(([position, length]) => {
    ...
    static isNonOverlapping(_normalPackMap) {
        return true;
    }
    static isOrdered(_normalPackMap) {
        return true;
    }
    /**
     ...
     */
    static normalizePackMap(packMap, warnStrict = true) {
        function formatE(kind, item) {
            return `invalid packMap format (${kind}): ${JSON.stringify(item)}`;
        }
        const normalPackMap = packMap.map(item => {
            if (Array.isArray(item)) {
                ...
                if (item.length === 1) {
                    if (warnStrict) {
                        console.warn('sloppy packMap format', item);
                    }
                    const [first] = item;
        ...
        if (!BitUtil.isOrdered(normalPackMap)) {
            if (warnStrict) {
                console.warn('sloppy packMap format (order)');
            }
        }
    ...
    static reconstruct10bit(msb, lsb_2bit) { return BitUtil.reconstructNBit(TEN, [msb, lsb_2bit]); }
    static reconstruct12bit(msb, lsb_4bit) { return BitUtil.reconstructNBit(TWELVE, [msb, lsb_4bit]); }
    static reconstruct20bit(msb, lsb, xlsb) { return BitUtil.reconstructNBit(TWENTY, [msb, lsb, xlsb]); }
}
